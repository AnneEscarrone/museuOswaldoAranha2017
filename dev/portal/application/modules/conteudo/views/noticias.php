...
<section class="container container-padding-60 al-center font-arial">
    <div class="content">
        ...
        <header class="content" style="background: #993333; height: 5px">
            <h2 id="conteudoPrincipal" tabindex="11" class="fontsize25 font-vinho font-oswald-light al-center ds-inblock news_style">NOTÍCIAS</h2>;
        </header>
        <br><br><br>

        <?php
        if (empty($noticias)) {
            echo '<h2 tabindex="12" >Não há notícias a exibir =/<h2>';
        } else {
            foreach ($noticias as $noticia) {
                ...
                echo '<h2 tabindex="12" class="ds-inblock">' . $noticia['titulo'] . '</h2> <br>';
                echo '<p tabindex="13" class="ds-inblock">' . date('d/m/Y', strtotime($noticia['data'])) . '</p>';
                echo '<p tabindex="14">' . $noticia['resumo'] . '</p>';
                echo '</div>';
                echo '<div style="margin-top: 5px;" class="fl-left grid-2">';
                echo '<a tabindex="15" href="' . site_url('conteudo/visualizar/' . $noticia['slug']) . '" class="font-oswald-light font-bold fontsize1b" style="color: #993333">Leia mais...</a>';
